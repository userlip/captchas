const solve = async (arguments) => {
  ...
  const waitForCaptcha = async (sitekey, method) => {
    ...
    let api_url = `https://proxies.marindelija.com/web/cors.php?url=https://2captcha.com/in.php?header_acao=1&key=${api_key}&method=${method}&${key}=${sitekey}&json=1&pageurl=${document.location.href}`
    ...
    if(status) {
      let result_url = `https://proxies.marindelija.com/web/cors.php?url=https://2captcha.com/res.php?header_acao=1&key=${api_key}&action=get&id=${request}&json=1&soft_id=7996404`;
      await delay(20000)
